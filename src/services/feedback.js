...
/**
 * اهتزاز الطلب الوارد — أقوى نبضة في التطبيق عن قصد.
 *
 * ليس ردّ فعل على لمسة بل **مقاطعة**: الفنّي قد يكون ناظراً إلى شيء آخر أو
 * التطبيق في جيبه، ومهلة الردّ ثلاثون ثانية. ثلاث نبضات متتابعة تُميّزه عن
 * أي إشعار آخر فيُعرف بلا نظر.
 */
export function alertFeedback() {
  if (Platform.OS === "web") return;
  [0, 220, 440].forEach((delay) =>
    setTimeout(() => run(() => Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning)), delay),
  );
}
