...
const MAP_HTML = [
  ...
  "    // تشويش GPS يهزّ الإحداثي والسيارة واقفة؛ عتبة صغيرة (مترين) تكفي لكبحه",
  "    // مع إبقاء التتبّع لحظياً: أدنى حركة للجوال تحرّك السيارة.",
  "    if (moved < 2) { shown = next; return; }",
  "",
  ...
  "      target = next;",
  "      if (prevFix && metersBetween(prevFix, next) < 2) return;",
